	...
	public function startCountdown()
	{
		...
		if(ret != FunkinLua.Function_Stop) {
			...
			startTimer = new FlxTimer().start(Conductor.crochet / 1000 / playbackRate, function(tmr:FlxTimer)
			{
				...
				{
					case 0:
						try { FlxG.sound.play(Paths.sound('intro3' + introSoundsSuffix), 0.6); } catch(e:Dynamic) { FlxG.sound.play(Paths.sound('intro3'), 0.6); }
						tick = THREE;
					case 1:
						countdownReady = createCountdownSprite(introAlts[0], antialias);
						try { FlxG.sound.play(Paths.sound('intro2' + introSoundsSuffix), 0.6); } catch(e:Dynamic) { FlxG.sound.play(Paths.sound('intro2'), 0.6); }
						tick = TWO;
					case 2:
						countdownReady = createCountdownSprite(introAlts[1], antialias);
						try { FlxG.sound.play(Paths.sound('intro1' + introSoundsSuffix), 0.6); } catch(e:Dynamic) { FlxG.sound.play(Paths.sound('intro1'), 0.6); }
						tick = ONE;
					case 3:
