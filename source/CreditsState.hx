...
class CreditsState extends MusicBeatState
{
	...
	override function update(elapsed:Float)
	{
		...
		if(!quitting)
		{
            ...
            for (item in grpOptions.members[curSelected])
		    {
            for (touch in FlxG.touches.list){		
    			if(controls.ACCEPT && (creditsStuff[curSelected][3] == null || creditsStuff[curSelected][3].length > 4) || touch.overlaps(item) && touch.justPressed &&  && (creditsStuff[curSelected][3] == null || creditsStuff[curSelected][3].length > 4)) {
